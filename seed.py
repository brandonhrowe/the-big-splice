...
def populate():
    list_of_meta_keys = ['collection', 'description',
                         'identifier', 'subject', 'title']
    list_of_probe_keys = ['duration', 'width', 'height', 'avg_frame_rate']
    for i in search_items('collection:Film_Noirs', list_of_meta_keys):
        ...
        duration, width, height, frame_rate = [
            probe_data[j] for j in list_of_probe_keys]
        film = Film.objects.find_or_create(collection=collection, description=description, identifier=identifier, tags=tags, title=title,
                                           file_name=file_name, url=url, timecodes=timecodes, duration=duration, width=width, height=height, frame_rate=frame_rate)[0]
        print(film)
